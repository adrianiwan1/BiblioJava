...
public class SprawdzanieUzytkownik
{
	...
	public static boolean CzyZablokowany(int PodaneID) // Wyszukiwanie inta - > ID
	{
		int i = 0;
		int Znalezione = 0;
		boolean Istnieje;
		...
		try
		{
			...
			do
			{
				...
				{
					int Odczyt = OdczytaneDane.GetIdUzytkownika(); //Wpisanie danej do int.
					String Ban =OdczytaneDane.GetCzyZbanowany();
					if(PodaneID == Odczyt && Ban.equals("tak")) // Porownanie odczytu.
					{
						Znalezione++;
					}
				} else
	...
	public static String IdNazwaCztelnik(int PodaneID) // Wyszukiwanie inta - > ID
	{
		int i = 0;
		int Znalezione = 0;
		boolean Istnieje;
